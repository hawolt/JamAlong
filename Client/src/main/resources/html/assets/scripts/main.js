...
function adjustAudioGain(value) {
    call('http://localhost:35199/v1/config/gain/'+value);
}

function host() {
    fetch('http://localhost:35199/v1/api/host')
...
function username(name,partyId) {
    call('http://localhost:35199/v1/api/namechange/'+name+'/'+partyId);
}

function join(partyId) {
    call('http://localhost:35199/v1/api/join/'+partyId);
}

function load(link){
    call('http://localhost:35199/v1/api/load?url='+btoa(link));
}
